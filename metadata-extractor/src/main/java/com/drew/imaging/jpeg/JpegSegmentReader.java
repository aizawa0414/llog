...
public class JpegSegmentReader
{
    ...
    private JpegSegmentData readSegments(@NotNull final BufferedInputStream jpegInputStream, boolean waitForBytes) throws JpegProcessingException
    {
        ...
        try {
            ...
            if (!readFromStream(jpegInputStream, headerBytes, 2))
...
//                    throw new JpegProcessingException("stream ended before segment header could be read");
                ...
                if (!readFromStream(jpegInputStream, segmentLengthBytes, 2))
...
//                    throw new JpegProcessingException("segment size would extend beyond file stream length");
                ...
                if (!readFromStream(jpegInputStream, segmentBytes, segmentLength))
                    throw new JpegProcessingException("Jpeg data ended unexpectedly.");
    ...
    private boolean readFromStream(@NotNull BufferedInputStream stream, byte[] buffer, int bytesRead) throws IOException
    {
    	int count = 40;
    	int readed = 0;
    	while (--count > 0) {
    		readed += stream.read(buffer, readed, bytesRead - readed);
    		if (readed == bytesRead)
    			return true;
